...
RSpec.describe 'schema' do
  before(:all) do
    ActiveRecord::Base.connection.change_table :users do |t|
      ...
      t.index 'lower(name)', unique: true, include: [:created_at]
      t.index 'lower(name), lower(email)', include: 'id, updated_at'
    end
  end
  ...
  after(:all) do
    ActiveRecord::Base.connection.change_table :users do |t|
      ...
      t.remove_index name: 'index_users_on_lower_name'
      t.remove_index name: 'index_users_on_lower_name_lower_email'
  ...
  it { is_expected.to include(%(t.index "lower((name)::text)", name: "index_users_on_lower_name", unique: true, include: ["created_at"])) }
  it { is_expected.to include(%(t.index "lower((name)::text), lower((email)::text)", name: "index_users_on_lower_name_lower_email", include: ["id", "updated_at"])) }
end
